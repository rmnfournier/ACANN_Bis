...
function data_generation(input_file)
...
if isfield(parameter_values,'beta')
    ...
    warning('Parameter beta not found, beta=1 was used')
    BETA=1;
end
%nb_data
...
if isfield(parameter_values,'omega0')
    OMEGA_0=parameter_values.omega0;
else
    warning('Parameter omega0 not found, omega0=50 was used')
    OMEGA_0=50;
end
%tau
if isfield(parameter_values,'nb_tau')
    ...
    warning('Parameter nb_tau not found, nb_tau=512 was used')
    NB_TAU=512;
end
%legendre method
...
%% Generate the A 
% The A(w) are Lorenztian centered at wr with a with of sigma

wr=rand(1,NB_DATA,NB_PICS)*OMEGA_0;
sigma=rand(1,NB_DATA,NB_PICS)*5+0.001;
%how much term does each data contain
R=randi(NB_PICS,NB_DATA,1);
for i=1:NB_DATA
    if (rand(1,1,1)>0.75) &&( R(i,1)~=1)
        wr(1,:,1)=0;
        sigma(1,:,1)=rand(NB_DATA,1)*5+2;
    end
end
...
omega=linspace(0,OMEGA_0,NB_OMEGA);
...
%one must make sure that the term do not become to big, 
kernel_handler = @(tau,x) exp(-x'.*tau);

G_handlers= @(tau)normrnd( -integral(@(x) kernel_handler(tau,x)*A_sum_handlers(x')',0,OMEGA_0,'ArrayValued',true,'AbsTol',integral_tol)./NORMALIZATION_FACTOR',NOISE_LEVEL);
...
for ii=1:length(nl(:,1))
    dlmwrite(strcat('../Data/Gl',output,'.dat'),[(0:NB_GL-1)',nl(ii,:)'.*sqrt(2*(0:NB_GL-1)'+1)*BETA,repmat(2e-9,NB_GL,1)],'-append','delimiter',' ')
end
